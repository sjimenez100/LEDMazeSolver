...
struct Node {
  ...
  // really just an easy check of positon
...
int GetDistance(const Node& nodeA, const Node& nodeB) {
  int distX = std::abs((float)(nodeA.position[1] - nodeB.position[1]));
  int distY = std::abs((float)(nodeA.position[0] - nodeB.position[0]));
...
uint8_t GetLowestfCostNodeIndex(const vector<Node*>& openSet) {
  ...
  for (int i = 1; i < openSet.size(); i++) {
    ...
    if (setNode->fCost < lowestNode->fCost || (setNode->fCost == lowestNode->fCost && setNode->hCost < lowestNode->hCost)) {
      lowestNode = setNode;
      index = i;
    }
  }
